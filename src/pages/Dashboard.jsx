...
import { Plus, Loader2, Wallet, Users, Menu, Download, LogOut } from "lucide-react"; // Added LogOut
...
export default function Dashboard() {
  ...
  const { data: debts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const sharedAccountsDebts = allDebts.filter(debt => {
        const debtOwner = debt.created_by;
        return user.sharedAccountsWith?.some(email => debtOwner === email);
      });
      
      const individuallySharedDebts = allDebts.filter(debt => 
        debt.sharedWith?.includes(user.email) && debt.created_by !== user.email
      );
      
      const uniqueDebts = [...myDebts];
      [...sharedAccountsDebts, ...individuallySharedDebts].forEach(debt => {
        if (!uniqueDebts.find(d => d.id === debt.id)) {
  ...
  const { data: payments = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const myDebtIds = debts.map(d => d.id);
      return allPayments.filter(p => myDebtIds.includes(p.debtId));
    },
    enabled: !!user && debts.length > 0
  });
  ...
  const { data: increases = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const myDebtIds = debts.map(d => d.id);
      return allIncreases.filter(i => myDebtIds.includes(i.debtId));
    },
    enabled: !!user && debts.length > 0
  });
  ...
  const calculateRemainingDebt = (debt) => {
    ...
    const debtIncreases = increases.filter(i => i.debtId === debt.id);
  ...
  const totalRemaining = debts.reduce((sum, debt) => 
    sum + calculateRemainingDebt(debt), 0
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto p-3 sm:p-6 relative z-10">
        ...
        >
          ...
          <p className="text-gray-600 text-base sm:text-xl font-medium px-4">
                ...
                return (
                  ...
                  >
                    <DebtCard
                      ...
                      onClick={() => navigate(createPageUrl('DebtDetails', `?id=${debt.id}`))}
                    />
                  </motion.div>
                );
              })}
